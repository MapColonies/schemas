...
function handleError(msg: string, id?: string): void {
  ...
  if (id) {
    ...
    if (fs.existsSync(path.join(directory, `${fileNameStart}.schema.json`))) {
      ...
      fs.existsSync(path.join(directory, `${fileNameStart}.schema.ts`))
    ) {
      file = `${fileNameStart}.schema.ts`;
    } else {
      throw new Error(
        `Could not find the file ${fileNameStart} referenced in the error`
      );
    }
  }
...
function validateRef(ref: string): boolean {
  ...
  if (!ref.startsWith('https://mapcolonies.com/')) {
    handleError(
      `Error validating ${ref}: $ref is incorrect, it should be in the following format: https://mapcolonies.com/directory/version`
    );
    return false;
  }
  ...
  if (
    !fs.existsSync(basePath + '.schema.json') &&
    !fs.existsSync(basePath + '.schema.ts')
  ) {
    handleError(
      `Error validating ${ref}: $ref is incorrect, could not find the file referenced`
    );
    return false;
  }
  return true;
...
async function validateSchema(schema: any, file: string) {
  ...
  if (
    schema.$id !==
    'https://mapcolonies.com/' +
      fileWithoutSchema.substring(0, fileWithoutSchema.indexOf('.schema'))
  ) {
    return handleError(
      `Error validating file: $id is incorrect, it should be in the following format: https://mapcolonies.com/directory/version`
    );
  }
...
async function validateTsFile(file: string) {
  const [error, schema] = await presult(import(`../${file}`));
  if (error) {
    handleError(`Error importing file: ${error.message}`);
    return;
  }
...
for (const directory of directories) {
  ...
  if (files.every((file) => file.isDirectory())) {
    directories.push(...files.map((file) => `${directory}/${file.name}`));
    continue;
  }
  ...
  for (const file of files) {
    await asyncLocalStorage.run({ directory, file: file.name }, async () => {
      ...
      if (fileNameParts[0] !== `v${fileOrderCounter++}`) {
        return handleError(
          `schema is not in the correct order, expected v${
            fileOrderCounter - 1
          } but got ${fileNameParts[0]}`
        );
      }

      switch (fileNameParts[2]) {
        case 'json':
          await validateJsonFile(`${directory}/${file.name}`);
          break;
        case 'ts':
          await validateTsFile(`${directory}/${file.name}`);
          break;
        default:
          return handleError(`Found a file that was not expected`);
      }
    });
  }
}
...
for (const [id, parentRefsSet] of refsMap.entries()) {
  for (const ref of parentRefsSet) {
    if (refsMap.get(ref) && isRefCircular(refsMap.get(ref)!, id)) {
      handleError(
        `Error validating ${id}: circular reference detected with ${ref}`,
        id
      );
      break;
    }
